...
#define OMX_PULL_REPLY_PAYLOAD_SIZE_OF_PACKET(x) (x-sizeof(struct omx_pkt_head)-sizeof(struct omx_pkt_pull_reply))
#define OMX_PULL_REPLY_PACKET_SIZE_OF_PAYLOAD(x) (x+sizeof(struct omx_pkt_head)+sizeof(struct omx_pkt_pull_reply))

#define OMX_MEDIUM_FRAG_PAYLOAD_SIZE_OF_PACKET(x) (x-sizeof(struct omx_pkt_head)-sizeof(struct omx_pkt_medium_frag))
#define OMX_MEDIUM_FRAG_PACKET_SIZE_OF_PAYLOAD(x) (x+sizeof(struct omx_pkt_head)+sizeof(struct omx_pkt_medium_frag))
 ...
 */
#define OMX_PULL_REPLY_LENGTH_MAX OMX_PULL_REPLY_PAYLOAD_SIZE_OF_PACKET(OMX_MTU)
...
#define OMX_MEDIUM_FRAG_LENGTH_MAX (				\
  OMX_MEDIUM_FRAG_PAYLOAD_SIZE_OF_PACKET(OMX_MTU) <= 4096	\
    ? OMX_MEDIUM_FRAG_PAYLOAD_SIZE_OF_PACKET(OMX_MTU)		\
    : ( OMX_MEDIUM_FRAG_PACKET_SIZE_OF_PAYLOAD(8192) > OMX_MTU	\
      ? 4096							\
      : 8192 )							\
)
